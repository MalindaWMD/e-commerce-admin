...
export default function Example({ defaultValue, onChange}) {
  ...
  const [selected, setSelected] = useState(getGroupFromValue(defaultValue, user_groups))
  ...
  return (
    <Combobox as="div" defaultValue={defaultValue} value={selected} onChange={handleOnChange}>
          ...
          <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {filteredGroups.map((group) => (
              ...
              >
                {({ active, selected }) => (
                  <>
                    <span className={classNames('block truncate', selected && 'font-semibold')}>{group.label}</span>
